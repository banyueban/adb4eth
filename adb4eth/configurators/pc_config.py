...
class PcConfigurator:
    ...
    def configure(self) -> list[DetResult]:
        ...
        # Windows 配置类操作需要管理员权限：非管理员直接 FAIL，不做任何写操作
        if self.ctx.platform == "windows" and not self.adapter.is_admin():
            results.append(
                DetResult(
                    "CFG",
                    "管理员权限",
                    ...
                    "FAIL",
                    "当前为非管理员运行",
                    "请右键以管理员身份运行 adb4eth 后重试（仅检测功能无需管理员）",
                )
            )
        ...
        if fresh.ip == self.ctx.pc_ip and self.ctx.pc_ip.startswith(self.ctx.debug_net):
            results.append(
                DetResult(
                    "CFG", "PC静态IP", True, "PASS", f"已配置 {fresh.ip}，跳过", ""
                )
            )
            self.ctx.results.extend(results)
            return results
        ...
        try:
            ...
            if not protected:
                self._rollback()
                results.append(
                    DetResult("CFG", "自动回滚", True, "WARN", "已回滚配置", "")
                )
    ...
    def _rollback(self):
        ...
        for name, snap in self.ctx.snapshot.iface_cfg.items():
            iface = (
                self.ctx.iface
                if name == (self.ctx.iface.name if self.ctx.iface else "")
                else None
            )
            if iface:
