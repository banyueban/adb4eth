...
STATUS_COLOR = {"PASS": PAL["ok"], "FAIL": PAL["fail"], "WARN": PAL["warn"], "SKIP": PAL["skip"]}


def _font(size=13, weight="normal", mono=False):
    return ctk.CTkFont(family=_MONO_FONT if mono else _APP_FONT, size=size, weight=weight)
...
def _labeled_entry(master, label, initial="", placeholder="", width=260):
    ...
    ctk.CTkLabel(row, text=label, font=_font(12), text_color=PAL["muted"],
                 anchor="w").pack(fill="x")
    ent = ctk.CTkEntry(
        row, width=width, height=32, font=_font(13),
        fg_color=PAL["panel2"], border_color=PAL["border"], text_color=PAL["text"],
...
class App(ctk.CTk):
    def __init__(self, platform: str | None = None):
        ...
        self.worker = GuiWorker(platform=platform)
        self._seen = 0
        self._last_ctx = None
        self._has_results = False
        self._platform = platform or ("windows" if sys.platform.startswith("win") else "macos")
    ...
    def _header(self):
        ...
        ctk.CTkLabel(
            title_box, text="adb4eth", font=_font(19, "bold"),
            text_color=PAL["accent"], anchor="w",
        ).pack(side="left")
        ctk.CTkLabel(
            title_box, text="有线 ADB 调试 · 自动检测与配置",
            font=_font(12), text_color=PAL["muted"], anchor="w",
        ).pack(side="left", padx=(12, 0))
        ctk.CTkLabel(
            title_box, text=f"[{self._platform}]", font=_font(11, mono=True),
            text_color=PAL["skip"],
        ).pack(side="left", padx=(10, 0))

        self.status_pill = ctk.CTkLabel(
            head, text="● 就绪", font=_font(13, "bold"), text_color=PAL["muted"],
            fg_color=PAL["panel2"], corner_radius=14, height=28,
        )
    ...
    def _left_panel(self):
        ...
        ctk.CTkLabel(
            panel, text="调试参数", font=_font(12, "bold"), text_color=PAL["muted"],
            anchor="w",
        ...
        self.btn_full = ctk.CTkButton(
            btns, text="开始检测并配置", font=_font(13, "bold"),
            fg_color=PAL["accent"], hover_color="#4CE89A", text_color="#06231A",
            height=38, corner_radius=10, command=lambda: self._start(full=True),
        )
        self.btn_full.pack(fill="x")
        self.btn_detect = ctk.CTkButton(
            btns, text="仅检测", font=_font(13),
            fg_color="transparent", border_width=1, border_color=PAL["border"],
            hover_color=PAL["panel2"], text_color=PAL["text"],
            height=34, corner_radius=10, command=lambda: self._start(full=False),
        )
        self.btn_detect.pack(fill="x", pady=(8, 0))
        self.btn_cancel = ctk.CTkButton(
            btns, text="停止", font=_font(13),
            fg_color="transparent", text_color=PAL["fail"], hover_color=PAL["panel2"],
        ...
        self.progress = ctk.CTkProgressBar(
            ...
            fg_color=PAL["panel2"], progress_color=PAL["accent"],
        )
        self.progress.pack(fill="x", padx=14, pady=(16, 0))

        ctk.CTkLabel(
            panel, text="运行日志", font=_font(12, "bold"), text_color=PAL["muted"],
            anchor="w",
        ).pack(fill="x", padx=14, pady=(16, 4))
        self.log = ctk.CTkTextbox(
            panel, fg_color=PAL["bg"], border_color=PAL["border"], border_width=1,
            font=_font(12, mono=True), text_color=PAL["muted"],
    ...
    def _rail(self):
        ...
        ctk.CTkLabel(
            rail, text="逐层检测链路  L1 → L7", font=_font(14, "bold"),
            text_color=PAL["text"], anchor="w",
        ).grid(row=0, column=0, sticky="w", pady=(0, 10))

        self.rail_scroll = ctk.CTkScrollableFrame(
            rail, fg_color="transparent", corner_radius=0,
            scrollbar_button_color=PAL["border"], scrollbar_button_hover_color=PAL["skip"],
        )
        ...
        self.empty_hint = ctk.CTkLabel(
            self.rail_scroll, text="等待开始 — 点击左侧「开始检测并配置」\n"
                                   "工具将逐层检测 L1 物理 → L7 ADB，并在配置后自动连接。",
            font=_font(13), text_color=PAL["skip"], justify="center",
        )
    ...
    def _footer(self):
        ...
        self.foot_text = ctk.CTkLabel(
            foot, text="状态栏：准备就绪", font=_font(12),
            text_color=PAL["muted"], anchor="w",
        )
        self.foot_text.grid(row=0, column=0, sticky="w", padx=18)

        self.foot_adb = ctk.CTkLabel(
            foot, text="ADB：未连接", font=_font(12, mono=True),
            text_color=PAL["muted"], anchor="e",
        )
    ...
    def _build_menu(self):
        try:
            ...
            m_file.add_command(label="导出 Markdown 报告…", accelerator="Cmd/Ctrl+S",
    ...
    def _gather(self):
        ...
        try:
            ...
            return None, None, None, None, "参数错误：请填写正确的网段（如 192.168.100）、PC/收银机 IP 与端口"
        if not (pc.startswith(net + ".") and reg.startswith(net + ".")):
            return None, None, None, None, "参数错误：PC 与收银机 IP 需在调试网段内"
        return net, pc, reg, int(port), None
    ...
    def _start(self, full: bool):
        ...
        if err:
            ...
            return
        self._reset_rail()
        if not self.worker.start(no_config=not full, debug_net=net, pc_ip=pc,
                                 reg_ip=reg, adb_port=port):
            return
        self._set_running(True)
        self._set_status("运行中", PAL["accent"])
        self._append_log(f"启动{'仅检测' if not full else '检测并配置'} → {pc} ↔ {reg}:{port}")
    ...
    def _export(self):
        ...
        path = filedialog.asksaveasfilename(
            defaultextension=".md", initialfile="adb4eth_report.md",
    ...
    def _reset_rail(self):
        for w in self.rail_scroll.winfo_children():
            w.destroy()
        self.empty_hint = ctk.CTkLabel(
            self.rail_scroll, text="检测中…", font=_font(13), text_color=PAL["skip"],
        )
    ...
    def _add_result(self, r):
        ...
        top.pack(fill="x")
        ctk.CTkLabel(
            top, text=r.layer, font=_font(11, mono=True),
            text_color=PAL["muted"], fg_color=PAL["panel2"], corner_radius=6,
            width=58, height=20,
        ).pack(side="left")
        ctk.CTkLabel(
            top, text=r.name, font=_font(13, "bold"), text_color=PAL["text"], anchor="w",
        ).pack(side="left", padx=(10, 0))
        ctk.CTkLabel(
            top, text=STATUS_TEXT.get(r.status, r.status), font=_font(11, "bold"),
            text_color=color, anchor="e",
        ).pack(side="right")

        if r.evidence:
            ctk.CTkLabel(
                body, text=r.evidence, font=_font(12, mono=True),
                text_color=PAL["muted"], anchor="w", justify="left", wraplength=560,
        ...
        if r.advice and r.status in ("FAIL", "WARN"):
            ...
            ctk.CTkLabel(
                adv, text="建议", font=_font(11, "bold"), text_color=color, anchor="w",
            ).pack(side="left")
            ctk.CTkLabel(
                adv, text=r.advice, font=_font(12), text_color=PAL["text"],
        ...
        try:
            ...
            while True:
                ...
                if kind == "stage":
                    ...
                    self.foot_text.configure(text="运行出错，请查看日志", text_color=PAL["fail"])
                elif kind == "summary":
    ...
    def _finish(self, ctx):
        ...
        if adb and adb.status == "device":
            ...
            self.foot_text.configure(text="有线 ADB 调试已就绪，可直接使用。", text_color=PAL["ok"])
            ...
            self.foot_text.configure(
                text=f"{len(fails)} 项失败，请按各卡片建议处理后重试。", text_color=PAL["fail"])
            if adb:
                self.foot_adb.configure(text=f"ADB：{adb.status}", text_color=PAL["fail"])
        elif adb:
            self._set_status("未完全就绪", PAL["warn"])
            self.foot_text.configure(text="检测无失败项，但 ADB 尚未就绪。", text_color=PAL["warn"])
